---
layout: post
title: "Going indie, step 5.1: Make your custom transitions reusable"
...
As I mentioned in [step 4](https://danielgauthier.me/2020/02/19/indie-4.html), the “transitioning delegate” responsibility often falls on the presenting view controller by default. In the most naive case, this means that every time you want to present something using a custom transition, you need to implement a bunch of delegate methods in your presenting VC, giving it responsibilities it probably shouldn’t really have. Furthermore, if you want to support an interactive dismissal gesture, you might end up implementing a gesture handler inside your presented view controller — its view _is_ where the gesture is happening, after all — and then figure out some way to hook up your gesture handling code to the interaction controller that your presenting view controller owns.
...
class StandardInteractionController: NSObject, InteractionControlling {
    ...
    @objc func handleGesture(_ gestureRecognizer: UIPanGestureRecognizer) {
        ...
        currentProgress = (translation.y / interactionDistance)

        switch gestureRecognizer.state {
            case .began: gestureBegan()
            case .changed: gestureChanged()
            case .cancelled: gestureCancelled()
            case .ended: gestureEnded()
            default: break
        }
    }
...
Of course, there’s a lot more than that going on in the full implementation, but this gives you the idea: our interaction controller is initialized with the presenting view controller, and is responsible for installing and responding to the gesture recognizer, allowing us to set `interactionInProgress` to `true` before having UIKit kick off the dismissal, and making it trivial to get subsequent gesture updates through our interaction controller to UIKit. 
